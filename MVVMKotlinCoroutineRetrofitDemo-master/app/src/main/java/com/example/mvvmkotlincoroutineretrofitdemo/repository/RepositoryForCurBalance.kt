    ...
    fun modelingSeriesForRateInPortfolio(
        ...
        if (ratesCurSuccessLiveData.value!![instrument]!!.size == daysQuantity.toInt() + 1) {
            for (i in 0..daysQuantity.toInt()) {
                ...
                while ((!flagTrades) and ((timeTrade<  time1 + DAY_IN_SEC * (i + 1)))) {
                    if ((!flagTrades) and (timeTrade >= time1 + DAY_IN_SEC * i)) {
                ...
                while ((!flagTrans) and ((timeTrans < time1 + DAY_IN_SEC * (i + 1)))) {
                    if ((!flagTrans) and (timeTrans >=  time1 + DAY_IN_SEC * i)) {
